...
LIBRARY_TESTS="condition error lock mutex queue read_write_lock repeating_thread support thread thread_attributes thread_pool";
...
run_test()
{
	...
	local TEST_EXECUTABLE="./cthreads_test_${TEST_NAME}";
...
run_test_with_input()
{
	...
	run_test_on_input_directory "libcthreads" "${TEST_DESCRIPTION}" "default" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "input" "${INPUT_GLOB}";
	local RESULT=$?;
