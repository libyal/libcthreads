# Script that builds libcthreads
#
# Version: 20260608
...
Else
{
	...
	If (-Not (Test-Path "${VSSolutionPath}"))
	{
		...
		Invoke-Expression -Command "& '${Python}' ${MSVSCppConvert} --output-format ${VisualStudioVersion} ${VSToolsOptions} msvscpp\libcthreads.sln 2>&1" | %{ "$_" }
	}
}
If (${VisualStudioVersion} -eq "2026")
{
	$VSSolutionFile = "${VSSolutionPath}\libcthreads.slnx"
}
Else
{
	$VSSolutionFile = "${VSSolutionPath}\libcthreads.sln"
}
